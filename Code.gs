const CONFIG = {
  ...
  ADMIN_SETUP_KEY: 'CHANGE_THIS_ONE_TIME_SETUP_KEY',
  SESSION_HOURS: 8,
  MAX_UPLOAD_MB: 6
};
...
function doPost(e) {
  try {
    ...
    switch (action) {
      ...
      case 'adminStatus': data = adminStatus_(); break;
      case 'setupAdmin': data = setupAdmin_(d); break;
...
function adminStatus_(){
  setupSheets_();
  const hasAdmin=rows_(SHEETS.USERS).some(r=>safeEq_(r[5],'ADMIN'));
  return {hasAdmin:hasAdmin, setupAvailable:!hasAdmin};
}
...
function setupAdmin_(d){
  ...
  try{
    setupSheets_();
    const key=String(d.setupKey||'').trim();
    const expected=String(CONFIG.ADMIN_SETUP_KEY||'').trim();
    if(!expected || expected.indexOf('CHANGE_THIS_')===0) throw Error('Administrator setup is not configured. Set ADMIN_SETUP_KEY in backend/Code.gs before using Admin Setup.');
    if(key!==expected) throw Error('Invalid administrator setup key.');
    const users=rows_(SHEETS.USERS);
    if(users.some(r=>safeEq_(r[5],'ADMIN'))) throw Error('An administrator account already exists. Use the normal Login page.');
    const name=String(d.fullName||'').trim(), email=email_(d.email), pass=String(d.password||''), confirm=String(d.confirmPassword||'');
    if(!name||!email||!pass||!confirm) throw Error('Complete all administrator setup fields.');
    if(pass.length<10) throw Error('Administrator password must be at least 10 characters.');
    if(pass!==confirm) throw Error('Passwords do not match.');
    if(!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)) throw Error('Enter a valid administrator email.');
    const adminId=String(d.adminId||'ADMIN001').trim() || 'ADMIN001';
    if(users.some(r=>safeEq_(r[1],adminId)||safeEq_(r[3],email))) throw Error('Administrator ID or email already exists.');
    append_(SHEETS.USERS,[now_(),adminId,name,email,hash_(pass),'ADMIN','ACTIVE']);
    return {created:true,userId:adminId,email:email,role:'ADMIN',message:'Administrator account created successfully. You can now log in.'};
  } finally {
